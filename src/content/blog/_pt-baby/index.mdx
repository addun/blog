...
## Step 2: Unblock and Reset the Pi's Bluetooth

Pairing from the Pi's terminal walks straight into a wall of `bluetoothctl` errors. They're all OS-level, and you clear them by working down the stack.

First, the soft block. `Failed to start discovery: org.bluez.Error.NotReady` usually means the kernel has software-locked the adapter. Check with `rfkill list`, then unblock it:

```bash
sudo rfkill unblock bluetooth
```

Next, the state lock. `org.bluez.Error.InProgress` is a deadlock on the Pi 3's Broadcom chip, often triggered when something else (like a Home Assistant Bluetooth integration) scans at the same time. Tear the interface down and reload the Pi's serial firmware controller:
...
If the adapter keeps dropping into `InProgress`, make sure **nothing else is scanning**. On this setup Home Assistant lives on a separate PC, so it isn't fighting the Pi for the radio - but a stray `bluetoothctl scan on` in another terminal will do it just as well.

</Note>

## Step 3: Pair Directly by MAC
...
## Step 4: Run MPD in User-Space, Next to PipeWire
...
That gives you a `media_player.pt_baby_speaker` entity. From there it's a normal media player, ready for automations - for example: _if the crib sensor detects crying for more than 60 seconds between 8 PM and 4 AM, wake the speaker, confirm the connection, and play a soft lullaby._
