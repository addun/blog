...
I wanted to play a lullaby from my phone through the speaker by the crib. Hit play, walked out of the room... and only a few steps later realized the music was streaming over Bluetooth. The moment I left range, the connection dropped and the lullaby stopped with it.

So I figured: what if the speaker just ran over Wi-Fi instead?

That's how this thing was born:

**Home Assistant → Wi-Fi → Raspberry Pi → Bluetooth → BT Speaker**
...
**You don't need a Raspberry Pi for this.** An **ESP32 running [ESPHome](https://esphome.io/)** can act as the same near-cradle bridge for a few dollars - it speaks A2DP over Classic Bluetooth and exposes a `media_player` straight to Home Assistant, no MPD or Wi-Fi hop required. That's the cheaper, lower-power option if you're buying hardware. I just happened to have a Raspberry Pi 3 sitting idle, so I used it - the rest of this guide follows the Pi path.
