...
1. **Port forwarding + dynamic DNS** - simplest, but it means your home router is directly reachable from the entire internet and your real home IP is public.
2. **A static public IP from your ISP** - works, but rarely worth the price (more on that below).
...
### 4. Caddy on the VPS - the public-facing hop
...
### 5. Caddy on the home server - routing to the right container
...
One reverse-proxy hop on the VPS (public, TLS-terminating), one more on the home server (private, per-service routing). Two hops, but the second one never leaves the LAN.
...
## Bonus: the same VPS as a whole-home VPN

Once you're renting a VPS anyway, it can double as an exit node for your whole home network - not just for exposing services inbound, but for routing outbound traffic too. Put WireGuard on it, connect your router (or individual devices) as clients, and every packet leaving your home network appears to come from the VPS's public IP and location instead.
...
This is genuinely useful if the VPS happens to be in a different country than you: rent it in the US, and your whole household's traffic can look like it's coming from the US - useful for region-locked content or just for not leaking your home country/ISP to every site you visit. It costs nothing extra since it's the same box already paying for itself as the reverse-tunnel endpoint.
