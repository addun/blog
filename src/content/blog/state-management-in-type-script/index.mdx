...
Now when we look into application - on UI nothing has changed but under the hood had changed everything.
...
All examples so far were quite simple.
Lets now create a application where user can see data, filters those data,
sort it and navigate through pages.

{/* add demo with all those features */}

### Extending loaded state

As the first step we need to add more information about
loaded state, like page number, active filters etc.

<Note type='HELPFUL'>

We want to paginate/filtering/sorting on a server.
We will not implement it in a browser.
